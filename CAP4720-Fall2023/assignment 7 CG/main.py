...
slide3 = gui.add_slider("shininess", 1, 128, 32, 1)
slide4 = gui.add_slider("K_s", 0, 1, 0.5, 0.01)
...
while draw:
    ...
    shininess = slide3.get_value()
    k_s = slide4.get_value()

    materialColor = material_color_picker.get_color()
    specularColor = specular_color_picker.get_color()
    ...
    model_matrix_left = pyrr.matrix44.multiply(translationLeft, scaling)
    model_matrix = pyrr.matrix44.multiply(translationMiddle, scaling)
    model_matrix_right = pyrr.matrix44.multiply(translationRight, scaling)
    ...
    #object 1
    shaderProgram["model_matrix"] = model_matrix_left
    ...
    shaderProgram["shininess"] = shininess
    shaderProgram["specular_color"] = specularColor
    shaderProgram["Object_id"] = 2
    ...
    #glUniformMatrix4fv(model_loc, 1, GL_FALSE, model_matrix_right)
    shaderProgram["model_matrix"] = model_matrix_right
    shaderProgram["materialColor"] = materialColor
    shaderProgram["K_s"] = k_s
    shaderProgram["ambient_intensity"] = ambient_intensity
    shaderProgram["Object_id"] = 3
