...
slide = slideName.add_slider("RotationZ", -90, 90, 0)
slide1 = slideName.add_slider("RotationY", -180, 180, 0)
slide2 = slideName.add_slider("RotationX", -90, 90, 0)
...
draw = True
while draw:
    zAngle = slide.get_value()
    yAngle = slide1.get_value()
    xAngle = slide2.get_value()
    
    zRotation = pyrr.matrix44.create_from_z_rotation(np.deg2rad(zAngle))
    yRotation = pyrr.matrix44.create_from_y_rotation(np.deg2rad(yAngle))
    xRotation = pyrr.matrix44.create_from_x_rotation(np.deg2rad(xAngle))
    
    temp = pyrr.matrix44.multiply(translation, zRotation)
    temp1 = pyrr.matrix44.multiply(temp, yRotation)
    temp2 = pyrr.matrix44.multiply(temp1, xRotation)
    model_matrix = pyrr.matrix44.multiply(temp2, scaling)

    matLocation = glGetUniformLocation(shader, "model_matrix")
    glUniformMatrix4fv(matLocation, 1, GL_FALSE, model_matrix)
